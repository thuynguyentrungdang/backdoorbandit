...
public class FikaBridge
{
    ...
    public delegate void SendPlantC4PacketEvent(Player player, string doorId, int c4Timer);
    ...
    { 
        ...
        SendPlantC4PacketEmitted?.Invoke(player, doorId, c4Timer); 
    }
    
    public delegate void SendSyncOpenStatePacketEvent(Player player, string objectId, int objectType);
    ...
    { 
        //Plugin.LogSource.LogDebug("Sending player position packet");
        SendSyncOpenStatePacketEmitted?.Invoke(player, objectId, objectType); 
    }
}